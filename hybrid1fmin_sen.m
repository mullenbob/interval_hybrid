% hybrid 1   particle swarm - sensitivity method
% written 2023-06-08
...
out =fopen(name+'_fmin_sen.out','w');

strainflag=2;   % if set to one, strains will also be calculated
fid=out;
%set angle range
arange=pi/4.; 
fprintf(out,'hybrid1 fmincon - sensitivity one angle with range %f\n Run on : %s/n',arange,datetime('now'));
...
starttime=cputime;
iii=0;
fprintf(out,'hybrid1opt fmincon - sensitivity one angle with range %f\n',arange);
% start analysis clock
tic
starttime=cputime;
A=[];
...
fprintf(out,' fmincon_sen solution min %15.10e max %15.10e  fvalue min %15.10e  time %s cpu time %f function evals %d\n', min(x),max(x),fval,toc,cputime-starttime,iii);
[x,fval,exitflag,output] = fmincon(@funy,x0,A,b,Aeq,beq,lb,ub,@nonlcon,options);
fprintf(out,' fmincon_sen solution min %15.10e max %15.10e  fvalue max %15.10e  time %s cpu time %f function evals %d\n', min(x),max(x),-fval,toc,cputime-starttime,iii);
