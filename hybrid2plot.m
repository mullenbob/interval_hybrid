% hybrid 2 plot   this will plot the response as a function of angle with
% no optimization using sensitivity analaysis for other interval values.
% written 2023-06-10  RLM
...
fprintf(out,'hybrid1plot response using sensitivity angle range %f\n',arange);
%
xplot=zeros(npoints,1);
y1plot=xplot;
y2plot=xplot;
ymax1=-999999.;
ymax2=-99999.;
for i1=1:npoints
    xplot(i1)=angle;
    y1plot(i1)=funx(angle);
    y2plot(i1)=-funy(angle);
    if (y2plot(i1) > ymax2)
        ymax2=y2plot(i1);
        amin1=angle;
    end
