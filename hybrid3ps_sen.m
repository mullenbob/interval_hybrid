...
out =fopen(name+'_ps_sen_ind_angles.out','w');
...
%program for independent angle values
global ndof;
global delta;
global nel;
global tx;
...
for i=1:nnd
    if (fxA(i) ~= 0.)
        anglex=angle(nangle);
ii=ii+1;
force(ii)=fxA(i)*cos(anglex)+fyA(i)*sin(anglex);
ii=ii+1;
force(ii)=fyA(i)*cos(anglex)-fxA(i)*sin(anglex);
nangle=nangle+1;
    else
        ii=ii+1;
        force(ii)=0.;
